...
function teamCards(type){let d=stats();d.sort((a,b)=>type==="avgConceded"?a[type]-b[type]:b[type]-a[type]);return d.slice(0,5).map((x,i)=>`<div class="rank-card"><small>${i+1}位</small><b>${type.startsWith("avg")?x[type].toFixed(2):x[type]}</b><strong>${x.team}</strong><small>${type==="avgGoals"?"平均得点":type==="avgConceded"?"平均被得点":"無失点"}</small></div>`).join("")}
...
function renderHome(){homeStandings.innerHTML=standingHTML(homeStandingMode.value,true);homeMatches.innerHTML=[...matches].sort((a,b)=>b.match_no-a.match_no).slice(0,6).map(matchRow).join("");homePlayerRanks.innerHTML=playerCards(homePlayerStat.value);homeTeamRanks.innerHTML=teamCards(homeTeamStat.value)}
